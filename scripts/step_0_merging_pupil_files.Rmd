...
```
## For loop to merge files for blinking data
```{r warning=FALSE}
...
blink_data_tsst <- merged_data %>%
  ...
  mutate(blink_identity = case_when(
    `start timestamp [ns]` >= pprep.start & `start timestamp [ns]` <= pprep.end ~ "p_prep",
    `start timestamp [ns]` >= cprep.start & `start timestamp [ns]` <= cprep.end ~ "c_prep",
    `start timestamp [ns]` >= pspeech.start & `start timestamp [ns]` <= pspeech.end ~ "p_speech",
    `start timestamp [ns]` >= cspeech.start & `start timestamp [ns]` <= cspeech.end ~ "c_speech",
    TRUE ~ NA_character_
  )) %>%
